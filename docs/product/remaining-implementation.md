...
| Music | Licensed uploads, AI selection using track-wide acoustic dynamics, adaptive listening samples and approved output timeline, mix-only AI revisions preserving the saved track and offset, volume curve, fades, ducking, rhythm estimates, track alignment and reviewable multi-cut beat alignment | Cut shifts are bounded and skip speech, locks, overlays and discontinuous source cuts |
...
| Platform adaptation | Five platforms, per-platform narrative instructions and duration ceilings, manually reorderable scenes, editable segments/title/copy/CTA, 4 aspect ratios, editable hook/closing CTA overlays (style, position, timing), per-platform caption mode/size/position/color from a clean companion master, cover timestamp, video/captions/package export | Shorts capped at 180 seconds and square/portrait; full-frame fit/padding; caption restyling requires a newly rendered master and cannot remove text embedded in source footage; no automatic publishing or comprehensive platform-policy guarantees |
| Quality revision | Five AI scores, consecutive-render total/category deltas with regression flags, actionable fixes and one automatic low-score revision proposal with mandatory per-issue responses, output-time mapping and EN/ZH review display | Comparison requires matching recorded model/rubric; no unattended rerender or best-version selection; application and final approval remain explicit |
