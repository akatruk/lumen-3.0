...
- [ ] Add deterministic diagnostics with tests for artificial splitting, genuine visual changes, repetitions and speech-safe long takes.
- [ ] Feed diagnostics into creative planning and replace clip-count pacing claims with visual-run measurements while preserving existing response compatibility.
- [ ] Show measured proposal diagnostics in review, distinguish warnings from quality scores, and explain that long complete speech can be intentional.
- [ ] Run targeted tests, typecheck/build and locale checks; publish after validation using existing rollback procedure.
