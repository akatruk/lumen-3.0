...
await test('save failure preserves draft and exposes retry',async({page,button,tool,draft,writes})=>{
 await tool('Effects');await button('Gentle zoom').click();await page.route('**/manual',async r=>r.request().method()==='PUT'?r.fulfill({status:409,json:{detail:'plan_changed'}}):r.fallback());await button('Save manual edits').click();await page.getByText('The plan changed. Reload saved edits before continuing.').waitFor();assert.equal((await draft()).clips[0].zoom_end,1.2);assert(await button('Save manual edits').isEnabled());
});
