...
import {MediaLibrary,AssetPlacement,type Asset,type ExternalBroll} from './MediaLibrary';
...
export function ManualEditor({
  ...
  return (
    ...
    >
      <h3>{t("Make your own cut", "自己决定怎么剪")}</h3>
      <p>
        {t(
      ...
      <MediaLibrary pid={pid} lang={lang} assets={assets} onChanged={loadAssets}/>
      <MusicPlan pid={pid} revision={revision} assets={assets.filter(a=>a.metadata.kind==='music')} lang={lang} disabled={blocked||dirty||!!edit.music?.locked} onApplied={async()=>{sessionStorage.removeItem(draftKey);await load();await onSaved()}}/>
      <StockLibrary pid={pid} lang={lang} onChanged={loadAssets} assets={assets} scene={{label:`${selected+1} · ${clip.start.toFixed(1)}–${clip.end.toFixed(1)}s`,context:[clip.text,...edit.captions.filter(c=>c.end>clip.start&&c.start<clip.end).map(c=>c[lang]||c.original)].filter(Boolean).join(' ').slice(0,1000),disabled:blocked||!!clip.locked}} onPlace={id=>{const asset=assets.find(a=>a.id===id);if(!asset||blocked||clip.locked)return;const length=Math.min(clip.end-clip.start,asset.metadata.duration,4);if(length<=0)return;clipChange(selected,{external_broll:{asset_id:id,start:0,end:length,source_start:0},cutaway:null,approved:false});}}/>
      ...
      <details open>
        <summary>
          {t(
            "1. Arrange clips, crop and add text",
            "1. 排列片段、裁剪和添加文字",
          )}
        </summary>
        <fieldset disabled={blocked} className="director-fieldset">
          {edit.clips.map((c, i) => (
            <article className="manual-clip" key={c.id||i}>
      ...
      <TimelineRegenerate assets={assets.filter(a=>a.metadata.kind!=='music')} pid={pid} revision={revision} clipId={clip.id} locked={clip.locked} disabled={blocked||dirty} lang={lang} onApplied={async()=>{sessionStorage.removeItem(draftKey);await load();await onSaved()}} />
      ...
      <div className="manual-actions">
        <span role="status">
          {dirty
            ? t("Unsaved manual edits", "手动剪辑尚未保存")
            : t("Saved state", "已保存状态")}
        </span>
        <button
          disabled={blocked}
        ...
        >
          {t("Reload saved edits", "重新加载已保存剪辑")}
        </button>
        <button
          className="secondary"
