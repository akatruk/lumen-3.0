...
export function PlanRenderReview({pid,revision,lang,disabled,onRender}:{pid:string;revision:number;lang:Lang;disabled:boolean;onRender:()=>Promise<void>}){
 ...
 <p className="error-box">{w('Этот режим использует утверждённые базовые рекомендации и сохраняет выбранные озвучку и музыку. Ручные визуальные эффекты и монтаж из редактора не включаются. Чтобы собрать их, вернитесь на вкладку «Монтаж».','This mode uses approved cleanup recommendations and retains the selected voiceover and music. Manual visual effects and the editor timeline are not included. To render those, return to Edit.','此模式使用已批准的基础建议并保留所选配音和音乐，不包含手动画面效果或编辑器时间线。如需制作这些更改，请返回编辑。')}</p>
