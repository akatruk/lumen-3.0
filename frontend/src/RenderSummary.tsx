...
export type RenderSummaryData={revision:number;source_duration:number;output_duration:number;removed_seconds:number;removed_ranges:number[][];near_original:boolean;captions:number;music:boolean;normalize:boolean;global_operations:string[];pending_proposals:Record<string,number>;clips:{id:string;start:number;end:number;source_start:number;source_end:number;operations:string[]}[]};
const stamp=(n:number)=>`${Math.floor(n/60)}:${(n%60).toFixed(1).padStart(4,'0')}`;
export function RenderSummary({data,lang}:{data:RenderSummaryData;lang:Lang}){
