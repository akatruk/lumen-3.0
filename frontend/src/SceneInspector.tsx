...
export function EffectPresets({lang,first,zoom,zoomEnd,x,y,xEnd,yEnd,transition,onChange}:{lang:Lang;first:boolean;zoom:number;zoomEnd?:number|null;x:number;y:number;xEnd?:number|null;yEnd?:number|null;transition?:string;onChange:(value:{zoom?:number;zoom_end?:number;x_end?:number;y_end?:number;transition?:'crossfade'|'cut'})=>void}){
 const w=(r:string,e:string,z:string)=>workspaceText(lang,r,e,z);
 const items=[{id:'push',Icon:ZoomIn,title:w('Плавное приближение','Gentle zoom','缓慢放大'),detail:w('1× → 1,2×','1× → 1.2×','1× → 1.2×'),active:zoom===1&&zoomEnd===1.2,patch:{zoom:1,zoom_end:1.2}},
