...
def propose(edit,accents,track_duration,speech,skipped=None):
    ...
    def skip(left,right,reason):
        if skipped is not None:skipped.append({'left_id':left.id,'right_id':right.id,'reason':reason,'cut_index':i+1})
    for i in range(len(result.clips)-1):
        left,right=result.clips[i:i+2]
        if left.locked or right.locked:
            skip(left,right,'locked');continue
        if has_layers(left) or has_layers(right):
            skip(left,right,'timed_layers');continue
        if abs(left.end-right.start)>.001 or right.transition!='cut':
            skip(left,right,'source_or_transition');continue
        output=sum(c.end-c.start for c in result.clips[:i+1]);old=left.end
        # Do not move an already aligned cut to a worse neighbouring accent.
        if min(abs(b-output) for b in beats)<=.025:
            skip(left,right,'already_aligned');continue
        candidates=sorted((b for b in beats if abs(b-output)<=.35),key=lambda b:abs(b-output))
        rejected=set()
        for beat in candidates:
            new=old+beat-output
            if new-left.start<.5 or right.end-new<.5:
                rejected.add('short_shot');continue
            lo,hi=sorted((old,new))
            # Reject even crossing a short spoken word, not just landing inside one.
            if any(c.start-.12<hi and c.end+.12>lo for c in speech):
                rejected.add('speech');continue
            ...
            break
        else:skip(left,right,'speech' if 'speech' in rejected else 'short_shot' if rejected else 'no_nearby_accent')
