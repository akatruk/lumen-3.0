...
def validate_proposal(result,snapshot,target,duration):
    ...
    for edge in ('start','end'):
        ...
        if abs(value-original[edge])>.001 and any(c['start']+.12<value<c['end']-.12 for c in [*snapshot.get('transcript',[]),*snapshot['edit'].get('captions',[])]):
