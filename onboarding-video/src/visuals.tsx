...
export const Motion:React.FC<{children:React.ReactNode}>=({children})=>{
 ...
 return <Interactive.Div name="界面演示" style={{position:'absolute',left:845,top:205,width:955,height:660,opacity:interpolate(frame,[0,10],[0,1],{extrapolateRight:'clamp'}),translate:interpolate(frame,[0,20],['55px 16px','0px 0px'],{extrapolateRight:'clamp',easing:Easing.bezier(.16,1,.3,1)})}}>{children}</Interactive.Div>
};
export const Headline:React.FC<{number:string;title:React.ReactNode;description:string}>=({number,title,description})=>{
 const frame=useCurrentFrame();
 return <Interactive.Div name="步骤标题" style={{position:'absolute',left:120,top:270,width:660,opacity:interpolate(frame,[0,8],[0,1],{extrapolateRight:'clamp'}),translate:interpolate(frame,[0,16],['0px 24px','0px 0px'],{extrapolateRight:'clamp',easing:Easing.bezier(.16,1,.3,1)})}}>
 <div style={{fontSize:28,fontWeight:650,letterSpacing:3,color:'#4f7e69',marginBottom:38}}>使用指南 / {number}</div>
 <div style={{fontSize:104,fontWeight:650,lineHeight:1.25,letterSpacing:-4}}>{title}</div>
 <div style={{fontSize:38,color:'#687970',marginTop:35,lineHeight:1.6}}>{description}</div>
 </Interactive.Div>
};
...
export const Product:React.FC<{small?:boolean}>=({small})=>{
 ...
 <div style={{position:'absolute',left:'10%',top:'31%',height:'46%',width:'80%',background:'#f0eedf',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',borderRadius:2,color:'#335340'}}><span style={{fontFamily:'Georgia',fontSize:small?22:36}}>lumen</span><span style={{fontSize:small?10:15,marginTop:8,letterSpacing:2}}>自然之美</span></div>
 </div></div>
};
export const Base:React.FC<{children:React.ReactNode}>=({children})=>{
 const frame=useCurrentFrame();const step=Math.min(3,Math.floor(frame/75));
 return <AbsoluteFill style={{background:'#f4f5ed',color:Ink,fontFamily:'"PingFang SC", "Hiragino Sans GB", sans-serif'}}>
 ...
 <div style={{position:'absolute',left:120,right:120,top:75,display:'flex',alignItems:'center',justifyContent:'space-between'}}><span style={{fontFamily:'Arial',fontSize:57,fontWeight:700,letterSpacing:-3}}>◒ lumen.</span><span style={{fontSize:26,padding:'13px 25px',border:'1px solid #cad5c3',borderRadius:99}}>10 秒上手 · 中文指南</span></div>
 {children}
 <div style={{position:'absolute',left:120,right:120,bottom:73,display:'flex',gap:22}}>{['上传视频','智能分析','选择建议','预览下载'].map((label,i)=><div key={label} style={{flex:1}}><div style={{height:4,background:'#dce4d5',borderRadius:3,overflow:'hidden',marginBottom:20}}><div style={{height:4,width:`${Math.max(0,Math.min(1,(frame-i*75)/75))*100}%`,background:Ink}}/></div><div style={{fontSize:27,color:i===step?Ink:'#899385',fontWeight:i===step?600:400}}><span style={{fontSize:21,marginRight:18}}>0{i+1}</span>{label}</div></div>)}</div>
 <div style={{position:'absolute',right:120,bottom:27,fontSize:17,color:'#8a9487'}}>界面示意 · 处理时间因视频而异</div>
 </AbsoluteFill>
};
